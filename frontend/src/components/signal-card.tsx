    ...
    <Card className={cn(
      ...
      <div className={cn("p-0 relative flex flex-col h-full uppercase-none")}>
        <div className="flex flex-col h-full justify-between gap-2.5">
          <div className="space-y-2.5">
            ...
            <div className="flex items-center justify-between relative">
              <div className="flex items-center gap-3 min-w-0">
                ...
                <span className="text-[10px] font-bold text-muted-foreground/50 uppercase tabular-nums shrink-0">
              ...
              {!isCompact && (
                <button
                  ...
                  className="absolute -top-1.5 -right-1.5 p-2 text-muted-foreground hover:text-primary transition-all disabled:opacity-40"
                >
                  {isBookmarking
                    ? <RefreshCw className="w-3.5 h-3.5 animate-spin" />
                    : <Bookmark className={cn("w-3.5 h-3.5", isBookmarked ? "text-primary fill-primary" : "text-muted-foreground/30 hover:text-muted-foreground")} />
                  }
                </button>
              )}

              {isCompact && (
                <div className="w-4 h-4 text-muted-foreground/30 hover:text-primary transition-all shrink-0">
                  <ChevronRight />
                </div>
              )}
            </div>
            ...
            <h3 className={cn(
              ...
              isCompact ? "text-[13px] pr-8 line-clamp-1" : "text-[14px] line-clamp-2 tracking-tight"
            )}>
              {signal.title}
            </h3>
            ...
            {!isCompact && (signal.aiSummary || signal.summary) && (
              <div className="p-2.5 bg-accent/10 rounded-md border border-border/10 relative group/brief overflow-hidden">
          ...
          {!isCompact && (
            <div className="flex flex-col gap-2.5 mt-auto pt-2.5 border-t border-border/10">
              <div className="flex items-center gap-2.5 min-w-0 flex-wrap">
                <Badge variant="outline" className="h-4.5 text-[8px] font-black tracking-widest uppercase px-1.5 bg-accent/20 border-border/10 text-muted-foreground/60 rounded-sm shrink-0">
                  {signal.aiCategory?.split('|')[0] || 'INTEL'}
                </Badge>
                {signal.aiProvider && PROVIDER_CONFIG[signal.aiProvider] && (
                  <Badge variant="outline" className={cn(
                    "h-4.5 text-[7.5px] font-black tracking-widest uppercase px-1.5 border-border/10 rounded-sm shrink-0",
                ...
                <div className={cn(
                  "px-1.5 py-0.5 rounded text-[10px] font-black tabular-nums whitespace-nowrap shrink-0",
              ...
              <div className="flex items-center justify-between mt-0.5">
                <button
                  ...
                  className="flex items-center gap-1.5 text-[10px] font-black uppercase tracking-widest text-muted-foreground/60 hover:text-primary transition-all"
                >
                ...
                <a
                  ...
                  className="flex items-center gap-1 text-[10px] font-black uppercase tracking-widest text-primary hover:text-primary/80 transition-all group/link"
                >
                  EXPLORE
