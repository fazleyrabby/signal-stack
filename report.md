...
15. [Key Concepts to Learn](#15-key-concepts-to-learn)
16. [Common Commands Reference](#16-common-commands-reference)
...
### 6.2 Environment-Based AI Routing

SignalStack supports environment-based routing to control AI provider usage:
...
1. **Local/Development Mode** (`NODE_ENV !== "production"` OR `AI_EXTERNAL_ENABLED=false`):
   ...
   - Retries up to 2 times if fails
   - If local fails → marks signal as failed
   - Never calls Groq or OpenRouter
   - Logs: `ai_local_only_mode`, `ai_provider_used`

2. **Production Mode** (`NODE_ENV = "production"` AND `AI_EXTERNAL_ENABLED=true`):
   - Tries `Local → Groq → OpenRouter` chain
   - Respects 60s cooldowns on 429 errors
...
The full provider chain is: **Local (llama.cpp) → Groq → OpenRouter**.
...
async summarize(signal) {
  ...
  // 1. Try local first (35s timeout managed by AbortController in local.provider.ts)
  if (localAiEnabled) {
    summary = await this.local.summarize(title, content); // no Promise.race wrapper
  }

  // 2. Try Groq if local failed
...
### 17.1 Container Crash: FeedService Not Exported
...
### 17.2 Disk Full: Container Cannot Write
